...
for m=32:1:M
    for n=32:1:N
        ...
        for u=0:1:(H-1)
            for v=0:1:(W-1)
                F(u+1, v+1, m, n) = 2 / sqrt(H*W) * beta(u+1) * beta(v+1) * sum(sum((target_8x8 .* basis_vector(:,:,u+1, v+1))));
            end
        end
        figure(3);
        subplot(9,8,1);
...
for m=1:1:M
    for n=1:1:N
        for u=0:1:(H-1)
            for v=0:1:(W-1)
                f_hat(((m-1)*H+1):(m*H),((n-1)*W+1):(n*W)) = f_hat(((m-1)*H+1):(m*H),((n-1)*W+1):(n*W)) + ...
                    2 / sqrt(H*W) * beta(u+1) * beta(v+1) * F(u+1, v+1, m, n) * basis_vector(:,:,u+1, v+1);
            end
        end
    end
end
